...
7. **Topic → file map** (update the matching pair in BOTH trees when the change actually touches that topic — terse, existing-file convention, not an essay). Skipping these is how #111 shipped with a LIVE_TODO line while `05_API.md` / `04_DATABASE.md` / `18_AI_ENGINE.md` still described a world without chat:
   - New/changed Fastify or Next.js API route → `docs/claude_project/05_API.md` **and** `docs/brain/05_API.md`
   - New table, column, index, RLS, or migration → `docs/brain/04_DATABASE.md` (authoritative schema) **and** a pointer/note in `docs/claude_project/04_DATABASE.md`; also append the filename to `docs/brain/16_MIGRATION_CHECKLIST.md` if a migration was applied
   ...
   - A standing rule ("never do X") → both `10_DECISIONS.md` files (Context/Decision/Rationale). A shipped ticket is not automatically a decision.
   - If the item was still listed as open in `docs/brain/LIVE_TODO.md`'s priority-queue section, strike or retarget that line in the **same** commit — a Closed, verified entry plus a still-open queue line is a contradiction.
...
- **Playwright login for authenticated pages needs `apps/web/.env.playwright.local` (gitignored, not present by default). If it's missing, do not attempt to guess or create a login — report "could not verify, no local test credentials" and stop, per Status honesty above.** Local Playwright against `next dev` (Next.js 16 Turbopack) must use `http://localhost:3000`, not `http://127.0.0.1:3000` — Chromium's `Origin: http://127.0.0.1:3000` on the HMR WebSocket is treated as cross-origin, hydration never runs, and the login form looks clickable but does nothing. This is a Next 16 dev-mode issue, not an app bug (#111 frontend verification, 2026-09-11).
...
- Follow-up work on the AI-chat feature (#111, shipped 2026-09-11 — `dcdc877` backend + `9f2aada` frontend), given its regulatory sensitivity (no buy/sell, no personalized-position advice)
...
- A LIVE_TODO line plus 09/08/14 is still not enough if the change added a route, table, Claude method, or UI component — those have their own files (see Live-status sync protocol item 7 above). #111 shipped with status docs updated and API/DB/AI-engine/components still describing a world without chat; that is the failure mode this item exists to prevent.
