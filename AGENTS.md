...
- **Playwright login for authenticated pages needs `apps/web/.env.playwright.local` (gitignored, not present by default). If it's missing, do not attempt to guess or create a login — report "could not verify, no local test credentials" and stop, per Status honesty above.**
