      ...
      <div className="max-w-[1440px] mx-auto">
        ...
        <div className="mb-10">
          ...
          <p className="text-on-surface-variant max-w-2xl mt-2 font-medium">
            Analyze historical market volatility markers and validate predictive
            models against real-world geopolitical events.
          </p>
        </div>
              ...
              {POPULAR.map((p) => (
                <div
                  ...
                  onClick={() => {
                    ...
                    else if (p.horizon === "48hr")
                      fromDate.setDate(fromDate.getDate() - 2);
                    else if (p.horizon === "7d")
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            {results.isDemo && (
              <div className="p-4 rounded-md bg-amber-400 text-black font-semibold mb-2">
                ...
                and should not be used for trading decisions. Real historical
                backtesting with live data will be available in Beta.
              </div>
            )}
            {/* Stats Grid */}
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              {[
                ...
                {
                  label: "MAX DEVIATION",
                  value: `${results.maxMovePct}%`,
                  color: "text-primary",
                },
                {
                  label: "MIN DEVIATION",
                  value: `${results.minMovePct}%`,
                  color: "text-on-surface",
                },
              ].map((stat) => (
                <div
                ...
                >
                  ...
                  <p
                    className={`text-2xl font-mono ${stat.color} font-extrabold tracking-tighter`}
                  >
                    {stat.value}
                  </p>
                </div>
                ...
                <button
                  onClick={() => {
                    ...
                    const rows = results.rows
                      .map(
                        (r) =>
                          `"${r.date}","${r.country}","${r.summary.replace(/"/g, '""')}",${r.movePct}%,${r.correct ? "CORRECT" : "DEVIATED"}`,
                      )
                      .join("\n");
                  ...
                  <tbody className="divide-y divide-outline-variant/5">
                    {results.rows.map((row, i) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 font-label text-xs font-bold text-on-surface uppercase tracking-tight">
                          {row.country}
                        </td>
                        <td className="px-6 py-4 text-xs text-on-surface/70 leading-relaxed font-medium">
                          {row.summary}
                        </td>
                        <td
                          className={`px-6 py-4 font-mono text-xs font-bold ${row.movePct >= 0 ? "text-primary" : "text-error"}`}
                        >
                          {row.movePct >= 0 ? "+" : ""}
                        ...
                        <td className="px-6 py-4 text-center">
                          <span
                            className={`material-symbols-outlined text-xl ${row.correct ? "text-primary" : "text-error"}`}
                            style={{
                              fontVariationSettings: row.correct
                                ? "'FILL' 1"
                                : "",
                            }}
                          >
