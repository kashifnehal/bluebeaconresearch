...
> ⚠️ UPDATED 2026-09-25 — the merge matcher's `SIMILARITY_THRESHOLD` (Jaccard on `summary` tokens, region-exact + country-compatible + 8h `event_date` window) was lowered 0.55 → **0.33**, and disclaimer boilerplate ("no geopolitical or financial market implications") was added to its stopword list, after a real missed merge (two same-event "US strikes Iranian oil tankers" articles scored 0.333) and a live check that a naive threshold-only drop would have collided with unrelated low-materiality signals scoring the same. Both the threshold and the 8h window are still placeholders, not tuned constants — full rationale in `apps/backend/src/workers/signal-merge.ts`'s header comment, schema pointer in `docs/brain/04_DATABASE.md`. The event-detail API/UI now also surfaces a **Timeline** (raw_events behind `raw_event_ids`, oldest-first) and a **Related Events** section (other signals: same `country` AND overlapping `commodity_impacts` asset, within a placeholder 7-day window of *this event's own* `event_date`, labeled reinforcing/conflicting/mixed by comparing `direction` per shared asset) — see `05_API.md` / `06_COMPONENTS.md`.
