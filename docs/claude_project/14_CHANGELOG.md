...
- **Known limitation carried forward:** the `/watchlist/[symbol]` drill-down still keys off `COMMODITIES`/`?commodity=` only, so a forex card links to a degraded drill-down.
