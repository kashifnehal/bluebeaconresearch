...
> ⚠️ UPDATED 2026-08-19 — Built, and it's not display-time filtering anymore — it's now a real merge at insertion time. Confirmed live this was a genuine cost problem, not just a cosmetic one: 120 exact-duplicate-text signal pairs found in a 500-row sample of real production data (same wire copy syndicated across different RSS feed URLs, each triggering its own Haiku classification and, for high-severity stories, its own separate Sonnet briefing call). New `insertOrMergeSignal()` (`apps/backend/src/workers/signal-merge.ts`) runs after classification, comparing the AI-classified summary (not raw title text) against recent same-region signals — classification itself is never skipped. A duplicate (same/lower severity) merges into the existing signal and reuses its briefing; a genuine escalation (higher severity) updates the existing signal and regenerates the briefing rather than creating a second row. Full design: `docs/brain/10_DECISIONS.md` ADR 010.
