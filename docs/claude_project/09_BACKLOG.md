...
| — | #121 follow-up: 1h/4h/24h/48h outcome checkpoints | — | **Done 2026-09-13** — `outcome-tracker.ts` writes all four horizons; `GET /v1/accuracy` still aggregates 48h only. Unique key widened to `(signal_id, asset, checkpoint_hours)`. No `/accuracy` time-horizon UI. See `LIVE_TODO.md`, `14_CHANGELOG.md` v0.64.0/PHASE 31. |
