...
- **`signals`**: 20+ signals in 24h `event_date` window. New rows appear when RSS/GNews finds articles not yet in DB.
...
Insert into raw_events + signals (event_date = article PUBLISH time)
        ↓
Next.js /api/signals (24h event_date filter, sort by event_date DESC)
        ↓
