...
Server component, no props, `data-testid="legal-disclaimer-footer"`. Renders the standing not-investment-advice / not-a-registered-adviser disclaimer text in muted small `font-mono` type matching the existing `/status` and `/accuracy` footer treatments. Mounted in `(dashboard)/layout.tsx` (after `<main>`, wrapped in a `pb-[60px] md:pb-0` div so it clears the fixed `MobileTabBar` on mobile) so it appears on every page under the `(dashboard)` route group, and directly in `app/status/page.tsx` + `app/accuracy/page.tsx` — the only two pages that render `PublicHeader.tsx`; no shared public layout file exists for those two to route through instead.
...
**2026-09-26 (`73bcd34`):** `tensionHistory` prop removed (the sparkline it fed no longer exists — see §3.5c); breakdown bars removed, replaced with the same real high-severity tally text as desktop. New `clusterFilterCount`/`onClearClusterFilter` props — a "SHOW ALL" affordance mirroring the desktop panel's, shown when `map/page.tsx`'s `clusterStreamSignals` state is narrowing the shared `liveItems` list to a map cluster/point selection (see §3.5g).
...
### 3.5g MapPage cluster/point click → Intelligence Stream (`app/(dashboard)/map/page.tsx`) (2026-09-26, `73bcd34`)

- New `clusterStreamSignals: GeoSignal[] | null` state. `null` = the Intelligence Stream sidebar shows the normal paginated feed (unchanged behavior); non-null narrows `liveItems` (shared by the desktop panel and `MobileTensionSheet`) to just that selection, and disables the "load more" pagination while active. Cleared on: filter change, background map click (no feature hit), or the new "SHOW ALL" button.
- Cluster click (`map.on("click", "clusters", ...)`): calls `source.getClusterLeaves(clusterId, Infinity, 0, cb)` and maps the returned leaf ids back to the already-loaded `geolocatedSignalsRef.current` (not `signalFromMapProps`, so full signal data — impacts, summary, etc. — is available, not just the trimmed GeoJSON properties). Separately, `getClusterExpansionZoom()`'s zoom is refined via new module-level helper `refineClusterZoom()`, which inspects `getClusterChildren(clusterId)`: if the immediate child count is outside `[3, 8]`, it steps the target zoom by ±1 (for the >8 case, re-checks by summing grandchildren of any child that's itself a cluster — the `getClusterChildren` API has no analogous parent-lookup for the <3/zoom-out case, so that branch takes one step and stops). Capped at 2 total adjustment attempts; falls back to the un-refined zoom if nothing better is found.
- Unclustered-point click: same sidebar-population behavior for just that one signal, plus the existing popup (previously popup-only).
- `data-testid="map-cluster-click"` added on the map container div (the map itself renders to a single canvas — there's no per-cluster DOM node to tag).
- **Verified live (Playwright), both desktop and mobile widths:** single-point click correctly narrows the sidebar to "Showing 1 signal from map selection" and shows the popup; "SHOW ALL" clears it back to the full feed; background click also clears it. **Could not verify the cluster-leaves/zoom-refinement path end-to-end** — confirmed via direct calls that `source.getClusterExpansionZoom()`, `getClusterLeaves()`, and `getClusterChildren()` hang indefinitely and never invoke their callback for a live cluster in this dev environment (reproducible, isolated from any UI interaction). This is not new: the pre-existing cluster-click zoom handler (before this change) already called `getClusterExpansionZoom()` the same way, so cluster-click zoom was likely already silently non-functional. Unclear whether this is specific to this sandboxed harness (cross-origin Worker network access) or a real production defect — flagged in `LIVE_TODO.md`, not root-caused as part of this ship (out of this task's scope).
...
- **Chart attribution — "why did this happen" (#207/#228, 2026-09-26, Phase 1 DB-only):** the price chart's `<Line>` now has a custom `activeDot` — hovering (or tapping, on touch) the nearest data point shows a small circle with an invisible 16px-radius hit area, `data-testid="chart-attribution-trigger"`. Clicking it computes that point's own direction (`directionAtIndex()`: pct change vs. the adjacent point, `>=3%` → `volatile`, else `up`/`down` — same three-value vocabulary as `commodity_impacts[].direction`) and calls the new `GET /api/signals/attribution` (see `05_API.md`), rendering up to 3 results below the chart (`data-testid="chart-attribution-result"`, title + "N hours before this move" + link to `/events/[id]`) with the fixed framing line "Time-window observation only — not a claim that this event caused the move." beneath, or "No clearly related event found in BBR's tracked history for this window." when nothing scores above threshold. First-use-only explanatory banner ("Hover or tap a point to see what may have driven this move") above the chart, gated by the same `isHintSeen`/`markHintSeen` localStorage convention `FeatureHints.tsx` (3.8) uses (`bbr_hint_seen_chart_attribution`) rather than a new mechanism — reused directly rather than routed through the Driver.js pathname-based hint system, since this needs to attach to a dynamic per-point SVG element, not a static selector. Works identically across all 5 chart ranges (1M DB series + 6M/1Y/3Y/5Y Yahoo weekly). **Phase 1 only** — DB-only scoring, no external news fallback, no LLM call. Verified live (Playwright, standing test account) at 375px and 1366px: trigger appears on hover/tap, panel renders 3 results with correct "N hours before" text and working `/events/[id]` links, close button is 44×44 on mobile, zero horizontal overflow either width.
