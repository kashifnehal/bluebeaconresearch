...
- `20260912000000_signals_classification_method.sql` — reliability/trust fix. Adds `signals.classification_method` (`claude`|`heuristic`, indexed) + `classification_method_inferred` (boolean), plus a one-time backfill UPDATE in the same migration file (confidence-pattern match against heuristic's `dynamicConfidence` formula output set). Applied live to `evavcgfmemwryggdkjmx` via Supabase MCP `apply_migration`; verified via direct SQL — 1,722 rows backfilled `heuristic`/inferred, 1,124 left NULL, grouped counts confirmed post-apply. Security Advisor re-checked after: no new findings.
- `20260912120000_anthropic_daily_usage.sql` — #111 governance. Table `anthropic_daily_usage` (PK `usage_date, bucket`, RLS on / no policies). Applied live to `evavcgfmemwryggdkjmx` via Supabase MCP `apply_migration`; columns confirmed via `information_schema`. Security Advisor: new table appears in the existing `rls_enabled_no_policy` INFO list next to `service_health_events` / `raw_events` (intentional — service-role only).
- `20260912180000_profiles_notification_prompt_dismissed.sql` — #112. Adds nullable `profiles.notification_prompt_dismissed_at timestamptz`. Applied live to `evavcgfmemwryggdkjmx` via Supabase MCP `apply_migration` as `20260912133049`; column confirmed via `information_schema` (nullable, no default). Existing `profiles` RLS unchanged — user updates their own row.
- `20260912134700_user_channels_discord.sql` — Discord alert channel. Adds nullable `user_channels.discord_webhook_url` + `discord_connected_at`. Applied live to `evavcgfmemwryggdkjmx` via Supabase MCP `apply_migration` as `user_channels_discord`; columns confirmed via `information_schema`. Existing `user_channels_all_own` (FOR ALL, `user_id = auth.uid()`) left untouched — it already covers new columns on the same row. No CHECK added on `alert_rules.channels`.
- `20260913160000_signals_materiality_gate.sql` — #141. Eight `signals` columns (`relevance`/`novelty`/`event_category`/`market_mechanism`/`is_preview`/`source_confirmation`/`materiality_pass`/`materiality_reasoning`) + CHECKs + `signals_materiality_pass_idx`. Applied live to `evavcgfmemwryggdkjmx` (see `LIVE_TODO.md` #141). Listed here because #142's follow-up migration depends on that ship.
- `20260913180000_media_impact_watchlist.sql` — #142. Table `media_impact_watchlist` + unique `entity_name` + tier CHECK + partial active index + RLS public-read policy (`to anon, authenticated using (true)`, no write policy) + `signals.media_impact_entity`. Applied live to `evavcgfmemwryggdkjmx` via Supabase MCP `apply_migration` as `media_impact_watchlist`. Verified: 7 seeded rows / correct tiers; column present; Security Advisor does not list this table under `rls_enabled_no_policy`.
- `20260913190000_signal_outcomes_checkpoint_unique.sql` — #144. Drop `UNIQUE (signal_id, asset)`, add `UNIQUE (signal_id, asset, checkpoint_hours)`. Applied live to `evavcgfmemwryggdkjmx` via Supabase MCP `apply_migration` as `signal_outcomes_checkpoint_unique`. Verified via `pg_constraint` + worker write of 1/4/24/48 rows. Existing 48h values unchanged.
- `20260919143000_media_impact_watchlist_individual_social.sql` — #142 data update. UPDATE Elon Musk evidence to the 2018 "funding secured" / SEC record; INSERT `Compromised official social-media account` (2013 AP Twitter hack). No schema/RLS change. Applied live to `evavcgfmemwryggdkjmx` via Supabase MCP `apply_migration` as `media_impact_watchlist_individual_social`. Verified: 8 active rows, 2 `individual_social_media`, both new `markets` arrays empty.
- `20260919160000_profiles_is_test_account.sql` — #146. `profiles.is_test_account boolean not null default false` + UPDATE revoke/trigger; `handle_new_user()` copies from `raw_app_meta_data`; `is_real_user(uuid)` + `admin_usage_metrics()` exclude the flag. Applied live to `evavcgfmemwryggdkjmx` via Supabase MCP `apply_migration` as `profiles_is_test_account`. Verified: 10 seeded confirmed users, founder-console snapshot unchanged vs pre-seed.
