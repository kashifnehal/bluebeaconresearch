...
> **📍 Doc status — authoritative schema as of 2026-09-20** (includes `feedback_submissions`, `search_content_embeddings`, `is_test_account`, materiality columns, `media_impact_watchlist`). `claude/23_TODO.md` is not in this repo.
...
- `is_test_account` (`boolean`, NOT NULL, default `false`) — added by `20260919160000_profiles_is_test_account.sql` (#146). Prospect/demo flag. Excluded from `admin_usage_metrics()` and digest eligibility. Not a privilege flag. Copied by `handle_new_user()` from `raw_app_meta_data` only (never `user_metadata`). Authenticated UPDATE of this column is revoked + trigger-blocked.
...
- **#141 materiality-gate columns** (migration `20260913160000_signals_materiality_gate.sql`, applied live 2026-09-13). Existing rows backfilled `materiality_pass = true`; the gate only governs future inserts. Indexed on `materiality_pass`.
  - `relevance` / `novelty` (`real`, nullable, 0–1 range-checked)
  - `event_category` (`text`, nullable, 9-value CHECK: `armed_conflict_security` | `sanctions_export_controls` | `trade_policy_tariffs` | `energy_supply_disruption` | `agriculture_food_security` | `central_bank_macro` | `shipping_logistics` | `official_communication` | `other_market_relevant`)
  - `market_mechanism` (`text`, nullable) — plain-language transmission path, or null; never invented in the UI
  - `is_preview` (`boolean`, NOT NULL, default `false`) — true only for a pure calendar-reminder with no new claim
...
- `id` (`uuid`, PK, default `uuid_generate_v4()`)
- `source` (`text`, NOT NULL, check: `gdelt`, `acled`, `newsapi`, `gnews`, `manual`, `rss`) — `rss` added 2026-09-26 (claude/237, `20260926053910_raw_events_source_add_rss.sql`).
  > **Collector-vs-constraint note — RESOLVED 2026-09-26 (claude/237).** The 2026-08-27 note below flagged `rss-collector.ts` mislabeling its own rows `source: "newsapi"`, making RSS and GNews rows indistinguishable. Fixed: `rss-collector.ts` now writes `source: "rss"`; the dedup cross-check in `title-prefilter.ts` (which relied on RSS/GNews sharing `newsapi` to catch cross-posted duplicates) was updated to group `['newsapi', 'rss']` as one bucket instead of relying on a shared literal value, so the intended cross-source dedup behavior is preserved. `acled` remains unused pending credentials; `manual` remains unused.
  > **Original 2026-08-27 note (historical):** the allowed set had five values but only two were ever written (`gdelt` 1895 / `newsapi` 323). `gnews-collector.ts` also wrote `source: "newsapi"` with a comment claiming the constraint didn't allow `gnews` — stale even then, since `008_fix_source_constraint.sql` had already added it. Flagged for a deliberate decision rather than a silent fix; see resolution above.
- `materiality_checked_at` (`timestamptz`, nullable) — added 2026-09-26 (claude/237, `20260926053739_raw_events_materiality_checked_at.sql`). Stamped by `logMaterialityRejection()` (`lib/materiality-gate.ts`) the moment any of the 5 live classify-then-insert paths (rss/gnews/gdelt/acled-collector, reconciliation.ts) gets a confirmed `materialityPass: false` for this row. `reconciliation.ts`'s candidate query now excludes `materiality_checked_at IS NOT NULL` rows regardless of age — previously a rejected row (no `signals` row ever created for it) was indistinguishable from a genuine crash-orphan and got re-classified every 30 min for its full 12h orphan-eligible life (confirmed: 23 repeat Claude calls/rejected article). A row that instead passes the gate isn't stamped — it doesn't need to be, since reconciliation already excludes it via the `raw_event_ids` signal-coverage `.overlaps()` check.
...
- `computed_at` (`timestamptz`, NOT NULL, default `now()`); **UNIQUE** `(signal_id, asset, checkpoint_hours)` (widened 2026-09-13 from `(signal_id, asset)` so 1h/4h/24h/48h rows can coexist; migration `20260913190000_signal_outcomes_checkpoint_unique.sql`); index on `signal_id`.
...
13b. **`20260919160000_profiles_is_test_account.sql`**: `profiles.is_test_account` (#146). Applied live 2026-09-19 as `profiles_is_test_account`. See Table 1.
