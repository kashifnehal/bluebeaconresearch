...
> ⚠️ UPDATED 2026-09-13 (#143) — Next.js `/api/signals` and `/api/signals/:id` now also map `eventCategory` / `marketMechanism` / `isPreview` (and `:id` now includes `currencyPairImpacts`) so the MARKET IMPACT ASSESSMENT box can render them. Fastify `/v1` already returned these via `select("*")` from #141; this is the BFF mapping only. `relevance` / `novelty` / `source_confirmation` / `materiality_pass` / `materiality_reasoning` remain stored but unmapped in the BFF (#143 leftover).
