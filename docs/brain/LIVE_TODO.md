...
- #138 Site-wide copy & error-state integrity pass — Phase 2 after #137. Grep-and-catalog
  every raw/fallback/empty/error string in `apps/web` (known candidates: "Supabase client
  not available", IngestionStatusBanner collector-health line, middleware auth-unavailable
  copy, `api-response.ts` no-data). Plus a short internal voice guide for empty/error
  states. Not started; do not fold into another UI ticket.
