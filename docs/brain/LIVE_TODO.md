...
- #120 Plain-language AI-writing rewrite (generateAnalysis() system prompt) — closed 2026-09-11
  (see "Closed, verified").
- #121 Real /accuracy page + outcome-tracker worker — **backend half closed 2026-09-11**
  (see "Closed, verified"): `signal_outcomes` table + daily `outcome-tracker.ts` worker,
  backfilled to 2,965 rows. **Frontend `/accuracy` page itself is still open** — still
  gated on the same "signal history long enough to be honest" judgment call (48h
  checkpoint means only signals from >=2 days ago are scored yet). #53 backfill ran
