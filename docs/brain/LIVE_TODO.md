...
## Closed, verified
- #140 hide raw classifier confidence on CommodityChip — 2026-09-13 (this commit). `apps/web` only. `CommodityChip` no longer renders a visible `{confidence}%` (was `size="md"` only; `sm` already hid it). Chip shows ticker + direction arrow only. `confidence` stays on the prop/API for gating and for the `aria-label`, now "USOIL up, model classification confidence 88%" so screen readers do not hear a price probability. Usage sites (`SignalCard`, `SignalQuickView`, `MapSignalPopup`, events/alerts/watchlist) do not reprint a per-asset percent beside the chip. Signal-level confidence elsewhere (dashboard featured, quick-view header, map popup header, landing "Neural Confidence") left as-is. **Verified:** `pnpm --filter web test` (signal-display + CommodityChip render). **Not this commit:** #139 research, #143 classifier/prompt work, landing-page "Detection Conf" custom pills (not `CommodityChip`).
