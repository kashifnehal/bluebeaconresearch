...
- #53 `commodity_impacts` historical backfill — production run 2026-09-11 (`8733677`). One-time script `apps/backend/src/scripts/backfill-commodity-impacts.ts` reuses `ClaudeService.classifyEvent()` (Haiku `claude-haiku-4-5-20251001`, same function the live collectors call) and writes **only** `commodity_impacts`. Live pipeline untouched. **Before:** 2,824 rows, 767 filled / 2,057 empty. **After:** 2,829 rows, 1,634 filled / 1,195 empty. Original empty set: 864 newly filled + 992 Haiku-classified as genuinely empty (checkpointed) + 201 skipped when Anthropic returned `credit balance is too low` (not written, not checkpointed). Cost quote before writes: $1.53–$2.77 (346 input tok, $1/$5 per MTok); account limits from live headers 10k RPM / 10M ITPM / 2M OTPM (Scale). Script aborts on that credit error on a re-run. Remainder is gated on restoring Anthropic credit, then `pnpm --filter backend backfill:commodity-impacts`. Smoke-test: Iran-ports row `bde8c96a-cf26-4b54-a4d7-a5c2eeb04527` wrote USOIL/UKOIL/NGAS; other columns unchanged.
...
- Anthropic credit topped up 2026-09-12 (~$6). Regular cron ingestion is safe
  under the new ingestion budget. Remaining 201 #53 rows + promoting #111 chat
  wait on this commit deploying and the founder confirming Railway
  `CHAT_ALLOWED_EMAILS` / budget values, then one manual allowlisted chat message.
...
- #121 Real /accuracy page + outcome-tracker worker — **fully closed 2026-09-11**
  ...
  + public `/accuracy` page, all live-verified with real numbers. #53 backfill ran
  2026-09-11 (filled 767→1,634 `commodity_impacts`); 201 historical empties remain
  because Anthropic credit exhausted mid-run.
